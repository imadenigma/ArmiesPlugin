...
class MenuGui(val user: User) {
    ...
    private fun addContent() {
        ...
        for (player in user.army.members) {
        ...
        user.player!!.inventory.addItem(skull)
        ...
        this.gui.setItem(13,
            ItemBuilder.from(Material.BEACON)
                ...
                .setLore("&9HP: ${user.army.hp}".colorize())
                .asGuiItem {
                    it.isCancelled = true
                    user.player!!.teleport(user.army.core.location)
                }
        )
        this.gui.setItem(14,
            ItemBuilder.from(Material.CHEST)
                .setName("&3Treasury".colorize())
                .setLore("&9balance: ${user.army.getBalance()}".colorize())
                .asGuiItem { it.isCancelled = true }
        )
        this.gui.setItem(15,
