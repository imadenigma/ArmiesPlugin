...
class PlayerListeners : Listener {
    ...
    fun onPlayerInteract(e: PlayerInteractEvent) {
        ...
        if (ItemNBT.getNBTTag(e.player.inventory.itemInMainHand, "isCard") == user.army.name) {
            if (!MainCommands.checkExistence(user, "name")) return
            ...
            if (army != null) {
                ...
                user.army.claimArea(e.player.location)
                e.player.sendMessage("you claim the area successfully")
                e.player.inventory.itemInMainHand.amount--
            }
        }
    ...
    fun onBlockBreak(e: BlockBreakEvent) {
        ...
        val user = User.getByUUID(e.player.uniqueId)
        e.isCancelled = true
        if (army.name.equals("US ARMY", true) || army.name.equals("SafeZone", true) || army.name.equals("War Zone", true)) {
            if (e.player.hasPermission("army.admin")) {
                e.isCancelled = false
                return
            }
        }
        if (!user.isOnArmy()) return
        when {
            e.block == army.core && user.army != army && (user.rank == Rank.EMPEROR || user.rank == Rank.KNIGHT || user.rank == Rank.SOLDIER) -> {
                army.takeDamage(user)
            }
            user.army == army && user.hasPermission(Permissions.BREAK) && e.block != army.core -> e.isCancelled =
                    false
        }
    }
    ...
    fun onLoadTurrets(e: PlayerInteractEvent) {
        ...
        if (nbt[1] != user.army.name) return
        ...
        when {
            ...
            nbt[0] == "sentry" && itemInHand.amount == config.getNode("sentry-upgrade-amount").getInt(1) -> {
                if (turret.get() is Sentry) {
                    turret.get().upgrade(user)
                    itemInHand.amount--
                }
            }
            nbt[0] == "gun" && itemInHand.amount == config.getNode("gun-upgrade-amount").getInt(1) -> {
                if (turret.get() is FireballTurret) {
                    turret.get().upgrade(user)
                    itemInHand.amount--
                }
            }
            nbt[0] == "manual" && itemInHand.amount == config.getNode("manual-upgrade-amount").getInt(1) -> {
    ...
    fun onPlayerMove(e: PlayerMoveEvent) {
        val user = User.getByUUID(e.player.uniqueId)
        val army = Army.getByLocation(e.player.location.x, e.player.location.z)
        user.isPvp = if (army?.name.equals("safezone", true)) false else army?.name?.equals("War Zone", true) == true
    ...
    fun onCoreHold(e: PlayerInteractEvent) {
        ...
        user.army.lastCoreHolding = System.currentTimeMillis()
        Schedulers.sync().runRepeating({ task ->
            if (!user.isOnArmy()) {
                task.close()
                return@runRepeating
            }
            if (user.army.core.type == Material.BEACON) {
                task.close()
                return@runRepeating
            }
            if ((System.currentTimeMillis() - user.army.lastCoreHolding) / 1000 < TimeUnit.MINUTES.toMillis(
                            minutes.toLong()
                    )) {
                task.close()
                return@runRepeating
            }
            user.army.withdraw(user.army.getBalance() * money / 100)
    ...
    fun onCorePlace(e: BlockPlaceEvent) {
        ...
        if (user.rank != Rank.EMPEROR || !user.isOnArmy()) return
        e.isCancelled = true
        val coreItem = getCoreItem(user.army)

        if (army?.name?.equals("US ARMY", true) == true || army?.name?.equals("SafeZone", true) == true || army?.name?.equals("War Zone", true) == true ) {
            if (e.player.hasPermission("army.admin")) {
                e.isCancelled = false
                return
            }
        ...
        if (!user.isOutsideArea && user.army != army) {
