...
class TurretListeners : Listener {
    ...
    fun onPlayerInteract(e: PlayerInteractEvent) {
        ...
        runCatching {
            if (user.army.name != nbt.split("-")[1]) return
        ...
        if (nbt.split("-")[0] == "sentry") {
            ...
            else Sentry(e.player.location, user.army, uuid = UUID.randomUUID())
            e.item.amount -= 1
            return
        }
        if (nbt.split("-")[0] == "gun") {
            ...
            else FireballTurret(e.player.location, user.army, uuid = UUID.randomUUID())
            e.item.amount -= 1
            return
        }
