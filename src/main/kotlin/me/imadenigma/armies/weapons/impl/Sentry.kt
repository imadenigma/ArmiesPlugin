...
class Sentry(
    ...
    override fun addAmmo(user: User, amount: Int) {
        for (content in user.player!!.inventory.contents) {
            content ?: continue
    ...
    override fun function() {
        ...
        val entity = this.location.world
            ...
            .filterIsInstance(Player::class.java).firstOrNull {
                ...
                else user.army != this.army && !this.army!!.allies.contains(user.army)
            } ?: return
        this.ammo--
    ...
    override fun takeDamage(user: User?) {
        ...
        if (bool) {
            ...
            if (!this.bossbar.players.contains(user.player!!)) this.bossbar.addPlayer(user.player!!)
            ...
            Schedulers.sync().runLater(
                {
                    if ((System.currentTimeMillis() - user.lastAgg) / 1000 >= 4 && this.bossbar.players.contains(user.player!!))
                        this.bossbar.removePlayer(user.player!!)
                }, 5L, TimeUnit.SECONDS)
        }
        if (this.hp <= 0) {
            if (bool) this.bossbar.removePlayer(user!!.player!!)
            this.despawn()
        }
    }
